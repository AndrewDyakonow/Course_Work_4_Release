    ...
    def button_click(self):
        ...
        try:
            if self.choice_servis.get() == '':
                ...
                JsonProcessing.create_file(self.input_tag.get(), self.choice_servis.get())
                self.list_vacancies = ProcessingData.get_vacancies_list(self.choice_servis.get())
                if len(self.list_vacancies) == 0:
                    raise ErrorNotData
                else:
                    for vacancies in self.list_vacancies:
                        self.output_field.insert(END, vacancies.beautiful_output() + f'\n{"=" * 50}\n')
    ...
    def button_top_click(self):
        ...
        try:
            if self.list_vacancies is None:
                ...
                if self.choice_servis.get() == "Head Hunter":
                    sort = sorted(self.list_vacancies, key=lambda d: (d.salary.to, d.salary.from_), reverse=True)
                else:
                    sort = sorted(self.list_vacancies, key=lambda d: (d.payment_to, d.payment_from), reverse=True)
